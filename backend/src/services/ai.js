...
export async function generateChannelCaption({ topic } = {}) {
  ...
  const prompt =
    ...
    `先判断这条视频属于哪种类型:若是一个人的自慰/打飞机,就往「单人自慰」方向写;若是两人或多人的性爱,就往「性爱」方向写;拿不准就按通用男同情欲写。\n` +
    `第一部分:一段有画面感、撩人、露骨的中文叙述(2~4 句,可用 emoji),贴合上面判断出的类型(自慰就写撸管/喷射快感,性爱就写抽插/激战),围绕男男视角,目的是吸引人点击观看完整视频。\n` +
    `第二部分:3~5 个与该类型高度相关的中文话题标签——自慰类用 #自慰 #打飞机 #撸管 之类,性爱类用 #性爱 #操 #内射 之类,并固定带上 #男同 #Gay;以 # 开头,用空格分隔,全部放在同一行。\n` +
