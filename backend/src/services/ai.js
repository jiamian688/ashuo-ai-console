...
export async function generateChannelCaption({ topic } = {}) {
  ...
  const t = (topic || '').trim();
  const defaultTags = (process.env.DEFAULT_TAGS || '#里番 #Hentai #成人动漫 #每日更新').trim();
  ...
  const kw = t
    ? `视频关键词/主题:「${t}」。`
    : '这是一条成人动漫(里番 / Hentai)短视频,没有特别指定关键词,请自行发挥写得撩人、通用。';

  const prompt =
    `你是一个成人动漫(里番 / Hentai)频道的文案写手,频道内容为男女向,受众是喜欢看成人动漫的成年人。` +
    `为一条要发到 Telegram 频道的短视频写中文引流文案。${kw}\n` +
    `按下面两部分输出,两部分之间用单独一行「===」分隔:\n` +
    `先判断这条视频属于哪种类型:若是女性单人的自慰,就往「单人自慰」方向写;若是男女之间的性爱,就往「男女性爱」方向写;拿不准就按通用男女情欲写。\n` +
    `第一部分:一段有画面感、撩人、露骨的中文叙述(2~4 句,可用 emoji),贴合上面判断出的类型(自慰就写手指/潮吹快感,性爱就写抽插/激战),围绕男女视角,目的是吸引人点击观看完整视频。\n` +
    `第二部分:3~5 个与该类型高度相关的中文话题标签——自慰类用 #自慰 #手指 #潮吹 之类,性爱类用 #性爱 #操 #内射 之类,并固定带上 #里番 #Hentai;以 # 开头,用空格分隔,全部放在同一行。\n` +
